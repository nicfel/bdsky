...
public class MultiRealRandomWalkOperator extends Operator {
    ...
    final public Input<Integer> scaleNeightboursInput =
            new Input<>("scaleNeightbours", "Use Gaussian to move instead of uniform interval. Default false.", 1);
   ...
   double windowSize = 1;
    ...
    public double proposal() {
        ...
        if (useGaussian) {
        	randVal = Randomizer.nextGaussian() * windowSize;
        } else {
        	randVal = Randomizer.nextDouble() * 2 * windowSize - windowSize;
        }

        
        int mid = Randomizer.nextInt(param.getDimension());
		for (int i = Math.max(mid-scaleNeightboursInput.get(), 0); 
				i < Math.min(mid+scaleNeightboursInput.get(), param.getDimension()); i++){
